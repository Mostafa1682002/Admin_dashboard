...
$userEdit = $connection->query("SELECT * FROM users WHERE UserID=$userID");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($newname)) {
        ...
        if (check_unique('users', 'Username', $newname, 'username_exist', "$newname Already exists,Please Add Anthor Username") > 1) {
            $Errors['username_exist'] = "$newname Already exists,Please Add Anthor Username";
        }
    }
    //validate Email
    if (empty($newemail)) {
    ...
    if (empty($Errors)) {
        ...
        $_SESSION['user_name'] = $newname; //Update Session['username'] 
    }
}
...
<div class="container">
    <form method="POST" class="formEdit" enctype="multipart/form-data">
        ...
        if (empty($Errors) && isset($_POST['edite'])) {
            echo "<div class='alert alert-success'>Updated Profile</div>";
        ...
        <img src="/Admin/includes/uploads/images_members/<?php echo $oldimge ?>" id="photo" class="photo show" alt="">
        <!-- Button Filed -->
        <div class="form-group my-3">
            <label for=""></label>
